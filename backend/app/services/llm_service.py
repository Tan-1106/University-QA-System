...
# Generate answer (Task 4.6 - Updated to handle conversation context)
async def generate_answer(
    ...
    Args:
        api_key: API key configuration {provider, api_key, using_model}
        chunks: Retrieved and reranked chunks
        question: Current user question
        question_language: 'vi' or 'en'
    ...
    if conversation_history and len(conversation_history) > 0:
        ...
        for msg in conversation_history[-20:]:  # Limit to last 20 messages (10 turns)
    ...
    if question_language == 'vi':
        ...
        2. Nếu có lịch sử hội thoại, hãy **tham khảo ngữ cảnh** từ các câu hỏi và trả lời trước đó để đưa ra câu trả lời có tính liên tục và phù hợp.
        3. Nếu văn bản là **phụ lục**, cần chú ý đến cấu trúc bảng: các thông tin trong cùng một hàng thuộc về cùng một đối tượng, và cần đọc theo thứ tự từ trái sang phải để hiểu đúng ý.
        4. Nếu thông tin liên quan có trong nhiều đoạn, hãy **tổng hợp và diễn đạt lại** thành một câu trả lời hoàn chỉnh.
        5. Nếu câu trả lời có nhiều ý hoặc được liệt kê dưới dạng danh sách. Hãy thêm "\n" vào giữa các ý để tiện cho quá trình render.
        6. Nếu có đáp án, thì ở cuối câu trả lời, hãy thêm mục **Nguồn tham khảo** gồm danh sách các tài liệu đã được sử dụng (mỗi mục gồm tiêu đề và URL ở cuối đoạn văn bản).
        7. Nếu **không tìm thấy** thông tin phù hợp trong các đoạn văn bản, hãy trả lời rằng không thể tìm được tài liệu trong kho dữ liệu liên quan đến câu hỏi của người dùng, không đề cập đến các tài liệu bạn được cung cấp và không cần dẫn nguồn tham khảo.
        8. Nếu người dùng đặt câu hỏi dựa trên 1 tình huống cụ thể, hãy suy luận câu trả lời dựa trên các đoạn văn bản được cung cấp nếu như các văn bản được cung cấp có thông tin liên quan đến tình huống đó. Không được trả lời dựa trên kiến thức chung hoặc kinh nghiệm cá nhân.
        9. Nếu người dùng cố gắng trò chuyện về các chủ đề không phù hợp hoặc ngoài phạm vi thay vì hỏi về nội dung thuộc phạm vi của Trường Đại học Tôn Đức Thắng, hãy trả lời một cách lịch sự rằng bạn chỉ có thể hỗ trợ các câu hỏi liên quan đến quy định, quy chế của Trường Đại học Tôn Đức Thắng và không được thiết kế để tham gia vào các cuộc trò chuyện ngoài phạm vi này, ngoài ra không cung cấp thông tin gì thêm về tài liệu nhận được từ hệ thống RAG.
        10. Không trả về các tài liệu tham khảo nếu câu trả lời được sinh ra không dựa trên các đoạn văn bản được cung cấp.
        {history_context}
        ...
        2. If there is conversation history, **refer to the context** from previous questions and answers to provide responses that maintain conversational continuity and relevance.
        3. If the text is **appendix**, pay attention to the table structure: information in the same row belongs to the same subject, and read from left to right to understand correctly.
        4. If relevant information is found in multiple passages, **synthesize and rephrase** it into a complete answer. The relevant information may be in Vietnamese, so make sure to translate your response completly to English.
        5. If the answer has multiple points or is listed as a list, add "\n" between the points for easier rendering.
        6. If there is an answer, at the end of the response, add a **References** section listing the documents used (each item includes the title and URL at the end of the passage).
        7. If **no relevant information** is found in the text passages, respond that you could not find documents related to the user's question in the database, do not mention the documents you were provided, and do not include a references section.
        8. If the user asks a question based on a specific situation, infer the answer based on the provided text passages if the provided texts contain information related to that situation. Do not answer based on general knowledge or personal experience.
        ...
        10. If the question is not in Vietnamese or English, politely inform the user that you can only process questions in Vietnamese or English. This response language is the question language if you can detect it, otherwise respond in English.
        11. Do not provide references if the generated answer is not based on the provided text passages.
        {history_context}
